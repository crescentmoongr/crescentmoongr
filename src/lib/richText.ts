...
export function sanitizeRichText(input: string | null | undefined) {
  ...
  // Normalize contenteditable block output before sanitizing.
  // Chromium often uses <div> for Enter; convert those to <p> so paragraph breaks are not lost.
  value = value
    .replace(/<div(?:\s[^>]*)?>/gi, '<p>')
    .replace(/<\/div\s*>/gi, '</p>');
...
function sanitizeWithLimit(input: string | null | undefined, limit: number) {
  ...
  // Preserve paragraphs generated by contenteditable Enter.
  value = value
    .replace(/<div(?:\s[^>]*)?>/gi, '<p>')
    .replace(/<\/div\s*>/gi, '</p>');

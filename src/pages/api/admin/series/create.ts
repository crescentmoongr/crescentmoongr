...
export const POST:APIRoute=async({request,cookies,redirect})=>{
  ...
  try{
    ...
    const rows=await supabasePost<Series[]>('series',token,{title,slug,description:clean(f.get('description'))||null,author:clean(f.get('author'))||null,artist:clean(f.get('artist'))||null,genres:clean(f.get('genres')).split(',').map(x=>x.trim()).filter(Boolean),cover_key:coverKey,type:clean(f.get('type'))||null,status,is_published:f.get('is_published')==='true',access_type:access});

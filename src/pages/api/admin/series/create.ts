...
export const POST:APIRoute=async({request,cookies,redirect})=>{
  ...
  try{
    const f=await request.formData(); const title=clean(f.get('title')); const slug=safeSlug(clean(f.get('slug'))); const status=clean(f.get('status')); const access=clean(f.get('access_type'))||'public'; const password=clean(f.get('series_password')); const rawUrl=safeUrl(clean(f.get('raw_url')));
    ...
    const rows=await supabasePost<Series[]>('series',token,{title,slug,description:sanitizeRichText(clean(f.get('description')))||null,author:clean(f.get('author'))||null,artist:clean(f.get('artist'))||null,raw_url:rawUrl,genres:clean(f.get('genres')).split(',').map(x=>x.trim()).filter(Boolean),cover_key:coverKey,type:clean(f.get('type'))||null,status,is_published:f.get('is_published')==='true',access_type:access});

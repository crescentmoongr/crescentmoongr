...
export const POST:APIRoute=async({request,cookies,redirect})=>{
  ...
  try{
    ...
    const title=clean(f.get('title')), slug=safeSlug(clean(f.get('slug'))), rawUrl=safeUrl(clean(f.get('raw_url'))); if(!title||!slug) throw new Error('Tên và slug là bắt buộc.');
    if(access==='password'&&password) await supabaseRpc('admin_set_series_password',{p_series_id:id,p_password:password},token);
    await supabasePatch(`series?id=eq.${encodeURIComponent(id)}`,token,{title,slug,description:sanitizeRichText(clean(f.get('description')))||null,author:clean(f.get('author'))||null,artist:clean(f.get('artist'))||null,raw_url:rawUrl,genres:clean(f.get('genres')).split(',').map(x=>x.trim()).filter(Boolean),type:clean(f.get('type'))||null,status:clean(f.get('status')),is_published:f.get('is_published')==='true',cover_key:coverKey,access_type:access});
